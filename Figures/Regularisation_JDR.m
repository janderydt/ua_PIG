function [RGrad,RAmpl]=Regularisation(UserVar,CtrlVar,MUA,BCs,F,l,Priors,Meas,BCsAdjoint,RunInfo)
...
if contains(lower(CtrlVar.Inverse.Regularize.Field),'cov')  % Bayesian regularization
    ...
    if isA
        if CtrlVar.AGlenisElementBased
            ...
            NAGrad=(gsA.^2.*(Dxx+Dyy))/Area;
            NAAmpl=(gaA.^2.*M)/Area;
        end
        RGradAGlen=dpA'*NAGrad*dpA/2;
        RAmplAGlen=dpA'*NAAmpl*dpA/2;
    ...
    if isC
        if CtrlVar.CisElementBased
            ...
            NCGrad=(gsC.^2.*(Dxx+Dyy))/Area;
            NCAmpl=(gaC.^2.*M)/Area;
        end
        
        RGradC=dpC'*NCGrad*dpC/2;
        RAmplC=dpC'*NCAmpl*dpC/2;
    ...
    RGrad=RGradAGlen+RGradC;
    RAmpl=RAmplAGlen+RAmplC;
    
 
end


RGrad=CtrlVar.Inverse.Regularize.Multiplier*RGrad;
RAmpl=CtrlVar.Inverse.Regularize.Multiplier*RAmpl;
