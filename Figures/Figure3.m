...
titlestr = {'{\boldmath${\cal E}_{\rm Calv}^3$}',...%\,{\rm \,(calving)}
    '{\boldmath${\cal E}_{\rm ISThin}^3$}',...%\,{\rm \,(ice\,shelf\,thinning)}
    '{\boldmath${\cal E}_{\rm Thin}^3$}',...%\,{\rm \,(thinning)}
    '{\boldmath${\cal E}_{\rm CalvThin}^3$}'};%\,{\rm\, (calving\,and\,thinning)}
barstr = {'${\cal E}_{\rm Calv}^3$','${\cal E}_{\rm ISThin}^3$ ',...
    '${\cal E}_{\rm Thin}^3$','${\cal E}_{\rm CalvThin}^3$'};
...
H = fig('units','inches','width',65*12/72.27,'height',90*12/72.27,'fontsize',16,'font','Helvetica');

%% calving, ice shelf thinning, thinning
ii=1;
for jj=1:3
    
    subplot('position',[0.08+0.3*(jj-1) 0.56 0.28 0.4]); hold on;
...
%         cb.Label.String = {'100(U_{2016}-U_{1996})/U_{2016} [%]'};
        cb=colorbar(gca,'Position',[0.11 0.92 0.22 0.015],'Location','northoutside','AxisLocation','in',...
         'Ticks',[0:50:200],'TickLabels',{'0','50','100','150','200'});
        cb.XColor='k';
        cb.YColor='k';
        cb.TickLength=0.04;
        cb.FontSize=16;
        cb.Label.String = '$(U_{\rm pert}-U_{96})/(U_{16}-U_{96}){\rm\, [\%]}$';%
    ...
    title(titlestr{jj},'interpreter','latex');

end

%% calving and thinning
subplot('position',[0.08 0.08 0.28 0.4]); hold on;
...
%% bar chart GL flux
subplot('position',[0.47 0.08 0.48 0.4]); hold on;
...
%       if jj==4
%           patch([perc perc 100 100],[(5-jj)-(-1)^ff*0.35 (5-jj) (5-jj) (5-jj)-(-1)^ff*0.35],[1 0.5 0.5]);%[1 0.5 0.5]
%       end
%        if jj==4 & ff==1
%            text(perc/2+50,5-jj-(-1)^ff*0.175,'unaccounted for','color','k','HorizontalAlignment','center');
%        end
        
%        patch([data(ii).FGFlux_1996(ff) data(ii).FGFlux_1996(ff) data(ii).Perturbation(jj).FGFlux(ff) data(ii).Perturbation(jj).FGFlux(ff)],...
%            [(5-jj)-(-1)^ff*0.35 (5-jj) (5-jj) (5-jj)-(-1)^ff*0.35],[0.5 0.5 1]);
%        text(data(ii).FGFlux_1996(ff)+data(ii).Perturbation(jj).dFGFlux(ff)/2,5-jj-(-1)^ff*0.175,...
%            [num2str(data(ii).Perturbation(jj).dFGFlux(ff)/data(ii).dFGFlux(ff)*100,2),'%'],...
%            'color','w','HorizontalAlignment','center');
%        if jj==4
%            patch([data(ii).Perturbation(jj).FGFlux(ff) data(ii).Perturbation(jj).FGFlux(ff) data(ii).FGFlux_2015_2016(ff) data(ii).FGFlux_2015_2016(ff)],...
%            [(5-jj)-(-1)^ff*0.35 (5-jj) (5-jj) (5-jj)-(-1)^ff*0.35],[1 0.5 0.5]);
%            text(data(ii).FGFlux_2015_2016(ff)/2+data(ii).Perturbation(jj).FGFlux(ff)/2,5-jj-(-1)^ff*0.175,...
