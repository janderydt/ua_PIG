...
titlestr = {'{\boldmath${\cal E}_{A}^3$}',...%\,{\rm \,(calving)}
    '{\boldmath${\cal E}_{C}^3$}'};
...
load([froot,'/Inversion_ERS_1996_REMAthickenedfinal_gs',num2str(gs),'_m',m,'_InverseRestartFile.mat']);
...
load([froot,'/Inversion_MEaSUREs_2015_2016_Bedmachine_gs',num2str(gs),'_m',m,'_InverseRestartFile.mat']);
...
%% 1996 data
load([froot,'/Inversion_ERS_1996_REMAthickenedfinal_gs',num2str(gs(1)),'_m3_InverseRestartFile.mat']);
...
%% 2016 data
%% 1. fixed C
load([froot,'/Inversion_MEaSUREs_2015_2016_Bedmachine_gs',num2str(gs(1)),'_m3_fixedC_InverseRestartFile.mat']);
MUA_1516 = MUA;
AGlen_1516 = F.AGlen;
...
%% 2. fixed AGlen
load([froot,'/Inversion_MEaSUREs_2015_2016_Bedmachine_gs',num2str(gs(1)),'_m3_fixedAGlen_InverseRestartFile.mat']);
C_1516 = F.C;
...
H=fig('units','inches','width',60*12/72.27,'fontsize',16,'font','Helvetica');

subplot('position',[0.1 0.1 0.85 0.85]); hold on;
 PlotNodalBasedQuantities(MUA.connectivity,MUA.coordinates/1e3,tb_diff);
 PlotGroundingLines(CtrlVar,MUA,GF,[],[],[],'-k','linewidth',2);
colormap(jet);
xlabel('psx [km]','fontsize',16); ylabel('psy [km]','fontsize',16);
caxis([-100 100]); 
cb = colorbar; cb.Label.String = 'Change in basal shear traction [kPa]';
set(cb,'fontsize',16);
grid on; box on;
...
fname = ['./Figure1_ReplyToReviewers'];
...
PlotNodalBasedQuantities_JDR(gca,MUA.connectivity,MUA.coordinates,C_diff,CtrlVar);
...
cb=colorbar(gca,'Position',[0.65 0.85 0.2 0.012],'Location','northoutside','AxisLocation','in',...
 'Ticks',[-0.5 -0.25 0 0.25 0.5],'TickLabels',{'-0.5','','0','','0.5'});
cb.XColor='k';
cb.YColor='k';
cb.TickLength=0.055;
cb.FontSize=16;
